...
docker run --rm \
  ...
  "$image_tag" \
  bash -lc 'nix shell nixpkgs#coreutils nixpkgs#git nixpkgs#glibc.bin nixpkgs#glibcLocales nixpkgs#libiconv nixpkgs#util-linux --command bash /source/tests/interactive-entrypoint.sh'
