...
grep -Eq 'DeepSeek API key +ready' <<<"$output" || {
  printf '%s\n' 'setup test: DeepSeek status is wrong' >&2
  exit 1
}
grep -Eq 'Pi account login +ready' <<<"$output" || {
  printf '%s\n' 'setup test: OAuth status is wrong' >&2
  exit 1
}
grep -Eq 'Claude Code +ready' <<<"$output" || {
  printf '%s\n' 'setup test: Claude Code status is wrong' >&2
  exit 1
}
grep -Eq 'Tailscale +ready' <<<"$output" || {
  printf '%s\n' 'setup test: Tailscale status is wrong' >&2
  exit 1
}
grep -Eq 'GitHub CLI +ready' <<<"$output" || {
  printf '%s\n' 'setup test: GitHub status is wrong' >&2
  exit 1
}
grep -Eq 'Cloudflare Tunnel +ready' <<<"$output" || {
  printf '%s\n' 'setup test: cloudflared status is wrong' >&2
  exit 1
}
grep -Eq 'Wrangler +ready' <<<"$output" || {
  printf '%s\n' 'setup test: Wrangler status is wrong' >&2
  exit 1
}
[ "$auth_before" = "$(file_hash "$home/.pi/agent/auth.json")" ] || {
...
[ "$conflict_status" -eq 1 ] || {
  printf '%s\n' 'setup test: shadowing key did not fail check' >&2
  exit 1
}
grep -Eq 'Pi API shadowing +deepseek' <<<"$conflict_output" || {
  printf '%s\n' 'setup test: shadowing key was not reported' >&2
  exit 1
}
[ "$conflict_before" = "$(file_hash "$home/.pi/agent/auth.json")" ] || {
...
grep -Fq 'DeepSeek: existing reference is ready.' <<<"$guided_output" || {
  printf '%s\n' 'setup test: existing key was not preserved' >&2
  exit 1
}
grep -Fq 'Gemini: found and verified llm-google.' <<<"$guided_output" || {
  printf '%s\n' 'setup test: Gemini was not auto-discovered' >&2
  exit 1
}
grep -Fq 'Moonshot: found and verified llm-moonshotai.' <<<"$guided_output" || {
...
[ "$(file_mode "$home/.config/proton-pass/pi.env")" = 600 ] || {
  printf '%s\n' 'setup test: reference mode is not 600' >&2
  exit 1
}
[ "$auth_before" = "$(file_hash "$home/.pi/agent/auth.json")" ] || {
  printf '%s\n' 'setup test: wizard modified Pi auth' >&2
  exit 1
}
! grep -q '^nvim ' "$logfile" || {
  printf '%s\n' 'setup test: wizard opened an editor' >&2
  exit 1
}
...
grep -Fq 'Gemini: found and verified llm-gemini.' <<<"$legacy_output" || {
  printf '%s\n' 'setup test: legacy Gemini title was not discovered' >&2
  exit 1
}
grep -Fq 'Moonshot: found and verified llm-moonshot.' <<<"$legacy_output" || {
  printf '%s\n' 'setup test: legacy Moonshot title was not discovered' >&2
  exit 1
}
...
grep -Fq 'Moonshot API key       skipped for now' <<<"$skipped_output" || {
  printf '%s\n' 'setup test: optional skip was not summarized' >&2
  exit 1
}
! grep -q '^MOONSHOT_API_KEY=' "$home/.config/proton-pass/pi.env" || {
  printf '%s\n' 'setup test: skipped key was written' >&2
  exit 1
}
grep -q '^GEMINI_API_KEY=' "$home/.config/proton-pass/pi.env" || {
  printf '%s\n' 'setup test: ready key was lost after skip' >&2
  exit 1
}
...
grep -Fq 'Pi account login       skipped for now' <<<"$account_output" || {
  printf '%s\n' 'setup test: Pi login skip was not summarized' >&2
  exit 1
}
grep -Fq 'Claude Code            ready' <<<"$account_output" || {
  printf '%s\n' 'setup test: Claude Code login was not completed' >&2
  exit 1
}
grep -Fq 'Tailscale              ready' <<<"$account_output" || {
  printf '%s\n' 'setup test: Tailscale login was not completed' >&2
  exit 1
}
grep -Fq 'Git identity           ready (Wizard User <wizard@example.com>)' <<<"$account_output" || {
  printf '%s\n' 'setup test: Git identity was not collected' >&2
  exit 1
}
[ -L "$home/.config/git/config" ] || {
  printf '%s\n' 'setup test: managed Git config symlink was replaced' >&2
  exit 1
}
[ "$(git config --file "$home/.config/git/identity" user.name)" = 'Wizard User' ] || {
  printf '%s\n' 'setup test: Git name was not written to the identity include' >&2
  exit 1
}
[ "$(git config --file "$home/.config/git/identity" user.email)" = 'wizard@example.com' ] || {
  printf '%s\n' 'setup test: Git email was not written to the identity include' >&2
  exit 1
}
grep -Fq 'GitHub CLI             ready' <<<"$account_output" || {
  printf '%s\n' 'setup test: GitHub login was not completed' >&2
  exit 1
}
grep -Fq 'Cloudflare Tunnel      ready' <<<"$account_output" || {
  printf '%s\n' 'setup test: cloudflared login was not completed' >&2
  exit 1
}
grep -Fq 'Wrangler               ready' <<<"$account_output" || {
  printf '%s\n' 'setup test: Wrangler login was not completed' >&2
  exit 1
}
grep -Fq 'claude auth login' "$logfile" || {
  printf '%s\n' 'setup test: Claude Code login command did not run' >&2
  exit 1
}
grep -Fq 'tailscale up' "$logfile" || {
  printf '%s\n' 'setup test: Tailscale login command did not run' >&2
  exit 1
}
grep -Fq 'gh auth login' "$logfile" || {
  printf '%s\n' 'setup test: GitHub login command did not run' >&2
  exit 1
}
grep -Fq 'cloudflared tunnel login' "$logfile" || {
  printf '%s\n' 'setup test: cloudflared login command did not run' >&2
  exit 1
}
grep -Fq 'wrangler login' "$logfile" || {
  printf '%s\n' 'setup test: Wrangler login command did not run' >&2
  exit 1
}
printf '%s\n' '{"openai-codex":{"type":"oauth","refresh":"must-survive"}}' >"$home/.pi/agent/auth.json"
auth_before=$(file_hash "$home/.pi/agent/auth.json")
...
rm -f "$mockbin/wrangler" "$mockbin/tailscale"
...
grep -Fq 'Tailscale              unavailable; install Tailscale through its signed system package repository' <<<"$missing_tool_output" || {
  printf '%s\n' 'setup test: missing Tailscale did not require a trusted manual installation' >&2
  exit 1
}
! grep -Fq 'tailscale.com/install.sh' "$logfile" || {
  printf '%s\n' 'setup test: missing Tailscale executed a remote installer' >&2
  exit 1
}
...
missing_node_status=$?
set -e
[ "$missing_node_status" -eq 1 ] || {
  printf '%s\n' 'setup test: missing Node.js did not fail status check' >&2
  exit 1
}
grep -Fq 'not checked; Node.js is unavailable' <<<"$missing_node_output" || {
  printf '%s\n' 'setup test: missing Node.js was not reported' >&2
  exit 1
}
! grep -q 'pass-cli run' "$logfile" || {
  printf '%s\n' 'setup test: status check invoked missing Node.js through Proton Pass' >&2
  exit 1
}
...
set -e
[ "$invalid_status" -eq 1 ] || {
  printf '%s\n' 'setup test: placeholder did not fail check' >&2
  exit 1
}
grep -Eq 'Pi API references +invalid' <<<"$invalid_output" || {
  printf '%s\n' 'setup test: invalid file was not reported' >&2
  exit 1
}
! grep -q 'pass-cli run' "$logfile" || {
  printf '%s\n' 'setup test: invalid reference reached Proton Pass' >&2
  exit 1
}
...
set -e
[ "$symlink_status" -eq 1 ] || {
  printf '%s\n' 'setup test: symlink did not fail check' >&2
  exit 1
}
grep -Eq 'Pi API references +invalid' <<<"$symlink_output" || {
  printf '%s\n' 'setup test: symlink was not reported' >&2
  exit 1
}
! grep -Fq 'ERROR:' <<<"$symlink_output" || {
  printf '%s\n' 'setup test: symlink aborted status reporting' >&2
  exit 1
}
