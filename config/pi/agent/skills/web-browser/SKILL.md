...
Adapted from [mitsuhiko/agent-stuff](https://github.com/mitsuhiko/agent-stuff/tree/main/skills/web-browser) for this Linux Pi setup. The bundled `LICENSE` contains the upstream Apache-2.0 license.

## Install Script Dependencies

After the skill is first applied, install its small CDP dependency once:

```bash
npm ci --prefix ~/.pi/agent/skills/web-browser/scripts --ignore-scripts
```
...
Chrome/Chromium is auto-detected on macOS, Linux, and Windows. If it is installed in a non-standard location, set:
