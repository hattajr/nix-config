...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/hattajr/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/Users/hattajr/Library/Application Support/tldraw/server.json'); TOKEN=$(jq -r .token '/Users/hattajr/Library/Application Support/tldraw/server.json')
