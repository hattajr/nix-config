...
const notify = (title: string, body: string): void => {
	...
	const osc = `\x1b]777;notify;${title};${body}\x07`;
	if (!process.env.TMUX) {
		process.stdout.write(osc);
		return;
	}

	// tmux passthrough: ESC P tmux ; <doubled ESC payload> ESC \\
	const passthrough = osc.replaceAll("\x1b", "\x1b\x1b");
	process.stdout.write(`\x1bPtmux;${passthrough}\x1b\\`);
};
