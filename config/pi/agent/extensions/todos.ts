...
import {
	compareTodoOrder,
	compareTodoSearchMatches,
	ensureTodoGitignore,
	isTodoClosed,
	tryAcquireTodoFileLock,
	writeTextFileAtomic,
} from "./todos/core.ts";
import {
	Container,
...
function filterTodos(todos: TodoFrontMatter[], query: string): TodoFrontMatter[] {
	...
	return matches.sort(compareTodoSearchMatches).map((match) => match.todo);
}
...
class TodoSelectorComponent extends Container implements Focusable {
	...
	private updateHeader(): void {
		...
		this.headerText.setText(
			this.theme.fg("accent", this.theme.bold("◈ Todo board")) +
				this.theme.fg("muted", ` · ${openCount} open`) +
				this.theme.fg("dim", " • ") +
				this.theme.fg("success", `${closedCount} done`),
		);
	}
	...
	private updateHints(): void {
		this.hintText.setText(
			this.theme.fg(
				"dim",
				"Type to filter • ↑↓ move • Enter menu • Ctrl+Shift+W work • Ctrl+Shift+R refine • Esc close",
			),
		);
	}
	...
	private updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i += 1) {
			...
			const titleColor = isSelected ? "accent" : closed ? "muted" : "text";
			const statusColor = closed ? "dim" : "success";
			const glyph = renderTodoGlyph(this.theme, todo, this.currentSessionId);
			const title = renderTodoTitle(this.theme, todo, titleColor);
			const tagText = todo.tags.length ? this.theme.fg("muted", ` [${todo.tags.join(", ")}]`) : "";
...
class TodoDetailOverlayComponent {
	...
	private buildTitleLine(width: number): string {
		const rawTitle = this.todo.title ? this.todo.title : `Todo ${formatTodoId(this.todo.id)}`;
		const titleText = ` ${rawTitle} `;
		const titleWidth = visibleWidth(titleText);
		const styledTitle = this.todo.title
			? this.theme.fg("accent", " ") + renderTodoTitle(this.theme, this.todo, "accent") + this.theme.fg("accent", " ")
			: this.theme.fg("accent", titleText);
		if (titleWidth >= width) {
			return truncateToWidth(styledTitle, width);
		}
...
async function garbageCollectTodos(
	...
	for (const entry of entries) {
		...
		if (!TODO_ID_PATTERN.test(id)) continue;
		...
		try {
			await withTodoLock(todosDir, id, ctx, async () => {
				...
				if (!todo || !isTodoClosed(todo.status)) return;
			...
			// Ignore unreadable or concurrently removed todos during best-effort GC.
		}
	}
}
...
async function ensureTodosDir(cwd: string, todosDir: string) {
	const overridePath = process.env[TODO_PATH_ENV];
	const usesDefaultRepoLocalPath = !overridePath || !overridePath.trim();
	if (usesDefaultRepoLocalPath) {
		const piDir = path.resolve(cwd, path.dirname(TODO_DIR_NAME));
		await fs.mkdir(piDir, { recursive: true });
		const gitignorePath = path.join(piDir, ".gitignore");
		await withFileMutationQueue(gitignorePath, () => ensureTodoGitignore(gitignorePath));
	}
...
async function writeTodoFile(filePath: string, todo: TodoRecord) {
	await writeTextFileAtomic(filePath, serializeTodo(todo));
}
...
async function acquireLock(
	...
	for (let attempt = 0; attempt < 2; attempt += 1) {
		const now = Date.now();
		try {
			const result = await tryAcquireTodoFileLock(lockPath, {
				id,
				pid: process.pid,
				session,
			...
			const owner = result.snapshot.info?.session
				? ` (session ${result.snapshot.info.session})`
				: "";
			const lockAge = now - result.snapshot.mtimeMs;
			if (lockAge > LOCK_TTL_MS) {
				return {
					error:
						`Todo ${displayTodoId(id)} has a stale lock${owner}. ` +
						`After verifying its owner stopped, remove ${lockPath} manually and retry.`,
				};
			}
...
function findOpenTodoByTitle(
	todos: TodoFrontMatter[],
	title: string,
): TodoFrontMatter | undefined {
	const normalizedTitle = normalizeTodoTitle(title);
	if (!normalizedTitle) return undefined;
	return todos.find(
		(todo) =>
			!isTodoClosed(getTodoStatus(todo)) &&
...
function renderTodoTitle(
	...
	const text = isTodoClosed(getTodoStatus(todo)) ? theme.strikethrough(title) : title;
	return theme.fg(color, text);
}

function renderTodoGlyph(theme: Theme, todo: TodoFrontMatter, currentSessionId?: string): string {
	if (isTodoClosed(getTodoStatus(todo))) {
...
function formatTodoHeading(todo: TodoFrontMatter): string {
	...
	const title = closed ? `~~${getTodoTitle(todo)}~~` : getTodoTitle(todo);
	const tagText = todo.tags.length ? ` [${todo.tags.join(", ")}]` : "";
	const glyph = closed ? "✓" : "○";
	return `${glyph} ${formatTodoId(todo.id)} ${title}${tagText}${formatAssignmentSuffix(todo)}`;
}
...
function renderTodoWidgetLines(
	...
	const mineCount = todos.filter((todo) => todo.assigned_to_session === currentSessionId).length;
	const headerParts = [
		theme.fg("accent", theme.bold("◈ Todos")),
		theme.fg("muted", `${openCount} open`),
		theme.fg("success", `${closedCount} done`),
	];
	if (mineCount > 0) {
		headerParts.push(theme.fg("accent", `${mineCount} mine`));
	}

	const lines = [truncateToWidth(headerParts.join(theme.fg("dim", " • ")), width)];
	const visibleTodos = todos.slice(0, 5);
	for (const todo of visibleTodos) {
		const closed = isTodoClosed(getTodoStatus(todo));
		const glyph = renderTodoGlyph(theme, todo, currentSessionId);
		const title = renderTodoTitle(theme, todo, closed ? "muted" : "text");
		const metaParts = [theme.fg("dim", formatTodoId(todo.id))];
		if (todo.tags.length) {
			metaParts.push(theme.fg("dim", `#${todo.tags.slice(0, 2).join(" #")}`));
		}
		if (todo.assigned_to_session === currentSessionId) {
			metaParts.push(theme.fg("accent", "you"));
		} else if (todo.assigned_to_session) {
			metaParts.push(theme.fg("dim", "busy"));
		}
		const meta = metaParts.length ? theme.fg("dim", " · ") + metaParts.join(theme.fg("dim", " · ")) : "";
		lines.push(truncateToWidth(` ${glyph} ${title}${meta}`, width));
	}
	return lines;
}
...
function renderTodoHeading(theme: Theme, todo: TodoFrontMatter, currentSessionId?: string): string {
	...
	return (
		renderTodoGlyph(theme, todo, currentSessionId) +
		" " +
		theme.fg("accent", formatTodoId(todo.id)) +
		" " +
		renderTodoTitle(theme, todo, closed ? "muted" : "text") +
		tagText +
		assignmentText
	);
}
...
function refreshTodoChrome(
	ctx: ExtensionContext,
	todosDir: string,
	todos: TodoFrontMatter[] = listTodosSync(todosDir),
): void {
	if (ctx.mode !== "tui") return;

	const openCount = todos.filter((todo) => !isTodoClosed(getTodoStatus(todo))).length;
	if (openCount === 0) {
		ctx.ui.setWidget("todos-board", undefined);
		ctx.ui.setStatus("todos-board", undefined);
		return;
	}

	const closedCount = todos.length - openCount;
	const currentSessionId = ctx.sessionManager.getSessionId();
	ctx.ui.setStatus("todos-board", `◈ ${openCount} open • ${closedCount} done`);
	ctx.ui.setWidget("todos-board", (_tui, theme) => ({
		render: (width: number) => renderTodoWidgetLines(theme, todos, width, currentSessionId),
		invalidate: () => {},
	}));
}
...
export default function todosExtension(pi: ExtensionAPI) {
	let todoWatcher: FSWatcher | undefined;
	let todoWatchTimer: ReturnType<typeof setTimeout> | undefined;

	const stopTodoWatcher = () => {
		if (todoWatchTimer) clearTimeout(todoWatchTimer);
		todoWatchTimer = undefined;
		todoWatcher?.close();
		todoWatcher = undefined;
	};

	const startTodoWatcher = (ctx: ExtensionContext, todosDir: string) => {
		stopTodoWatcher();
		if (ctx.mode !== "tui") return;
		try {
			const watcher = watch(todosDir, { persistent: false }, (_eventType, filename) => {
				if (filename && !String(filename).endsWith(".md")) return;
				if (todoWatchTimer) clearTimeout(todoWatchTimer);
				todoWatchTimer = setTimeout(() => refreshTodoChrome(ctx, todosDir), 75);
			});
			watcher.on("error", () => {
				watcher.close();
				if (todoWatcher === watcher) todoWatcher = undefined;
			});
			todoWatcher = watcher;
		} catch {
			// The board still refreshes after local mutations if watching is unavailable.
		}
	};

	pi.on("session_start", async (_event, ctx) => {
		const todosDir = getTodosDir(ctx.cwd);
		await ensureTodosDir(ctx.cwd, todosDir);
		const settings = await readTodoSettings(todosDir);
		await garbageCollectTodos(todosDir, settings, ctx);
		refreshTodoChrome(ctx, todosDir);
		startTodoWatcher(ctx, todosDir);
	});

	pi.on("session_tree", async (_event, ctx) => {
		refreshTodoChrome(ctx, getTodosDir(ctx.cwd));
	});
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			switch (action) {
				...
				case "create": {
					...
					const result = await withTodoLock(todosDir, "create", ctx, async () => {
						const existingSummary = findOpenTodoByTitle(
							await listTodos(todosDir),
							params.title,
						);
						if (existingSummary) {
							const existing = await ensureTodoExists(
								getTodoPath(todosDir, existingSummary.id),
								existingSummary.id,
							);
							if (existing) return { todo: existing, reused: true };
						}
					...
					refreshTodoChrome(ctx, todosDir);
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result.todo) }],
						details: { action: "create", todo: result.todo, reused: result.reused },
					};
				}
			...
			await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
				...
				const applyTodoAction = async (
					...
					if (action === "release") {
						...
						refreshTodoChrome(ctx, todosDir, updatedTodos);
